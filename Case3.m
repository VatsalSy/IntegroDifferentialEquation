...
clear
close all
%% Case 3: a''[t] == \int_0^t a'[x]dx With a[0] = 0
tmax = 2*pi;
...
while err > Tol
    [t,a] = ode113(@model, t, IC, odeOptions, guess);
    err1 = sum((a(:,1)-guess(:,2)).^2);
    err2 = sum((a(:,2)-guess(:,3)).^2);
    err = max(err1,err2);
    fprintf('  %4i: %8.2e\n',[counter err]);
    wt = 0.5;
    guess = [t (1-wt)*guess(:,2) + wt*a(:,1) a(:,2)];
    counter = counter+1;
end
...
annotation(figure1,'textbox',...
    [0.45 0.53 0.25 0.10],...
    'String','$\ddot{a} = \int_0^t\dot{a}(x)dx$',...
    'LineStyle','none',...
    'Interpreter','latex',...
...
ys = @(s) interp1(guess(:,1), guess(:,3), s);
